...
public class PacketHandlerReplicate {
  ...
  public PacketHandlerReplicate(@NotNull String json) {
    ...
    Cell[] updatedCells = new Cell[commandReplicate.getCells().length];
    ...
    for (int i = 0; i < commandReplicate.getCells().length; i++) {
      ...
      updatedCells[i] = new Cell(c.getX(), c.getY(), c.getSize(), c.getCellId(),c.getName());
    }
    ...
    Game.player.clear();
    Collections.addAll(Game.player, updatedCells);
    Game.cells = updatedCells;
